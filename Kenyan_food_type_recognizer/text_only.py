...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=10, shuffle=True, num_workers=0) ###, sampler = sampler
              for x in ['train', 'test']}
...
#                best_acc = epoch_acc
#                torch.save(model,"only_BERT.tar")
 #               best_model_wts = copy.deepcopy(model.state_dict())
...
class Img_text(nn.Module):
    ...
    def forward(self, texts):
        ...
        logits = self.output(all_pooled)
        return logits


K=3

model_ft = Img_text(13)
...
#plt.savefig('resnext101_bert_d7.png')
