...
f = open("../../img_text4.csv","r")
reader = csv.reader(f)
table = {i[0]:i[1] for i in reader}

class Image_text_Dataset(datasets.folder.ImageFolder):
    def __init__(self, root, transform=None, target_transform=None,
                 loader=datasets.folder.default_loader):
        super(Image_text_Dataset, self).__init__(root, transform, target_transform, loader)

    def __getitem__(self, index):
        path, target = self.samples[index]
        sample = self.loader(path)
#        text = table[path.split("/")[-1].strip(".jpg").split("_")[0]]
        classes_d = self.class_to_idx
        classes_d = {classes_d[i]:i for i in classes_d}
        text = table[path.split("/")[-1].strip(".jpg").split("_")[0]]
        text = text.replace("#"+classes_d[target],"")
        text = text.replace(classes_d[target],"")
        ...
        return sample, target, path, text


image_datasets = {x: Image_text_Dataset(os.path.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['train', 'test']}
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train','test']:
            ...
            # Iterate over data.
            for inputs, labels, im_paths, texts in dataloaders[phase]:
...
#                best_acc = epoch_acc
#                torch.save(model_ft,"resnext_bert_0.tar")
 #               best_model_wts = copy.deepcopy(model.state_dict())
...
class Img_text(nn.Module):
    def __init__(self, n_hidden, class_num):
        super().__init__()
...
#            param.requires_grad = False
        ...
        self.output = nn.Sequential(
            nn.Linear(768 + 2048, n_hidden),
            nn.ReLU(),
            nn.Linear(n_hidden, class_num)
        )
    ...
    def forward(self, texts, imgs):
        ...
        for text in texts:
            if not (len(text) == 0):
                ...
                tokens = tokens[:512]
                seqs = torch.tensor([self.tokenizer.convert_tokens_to_ids(tokens)],dtype=torch.long).to(device)
                _, pooled = self.bert(seqs, output_all_encoded_layers=False)
                all_pooled.append(pooled)
            else:
                pooled = torch.zeros([1,768],dtype=torch.float32).to(device)
                all_pooled.append(pooled)
        all_pooled = torch.cat(all_pooled)
        features = torch.squeeze(self.resnet(imgs))
        if len(features.size()) == 1:
            features = torch.unsqueeze(features,0)
        concat = torch.cat([all_pooled, features], dim=1)
...
im_paths = []
with torch.no_grad():
    for i, (inputs, classes, im_path, texts) in enumerate(dataloaders['test']):
...
im_paths = []
with torch.no_grad():
    for i, (inputs, classes, im_path, texts) in enumerate(dataloaders['val']):
